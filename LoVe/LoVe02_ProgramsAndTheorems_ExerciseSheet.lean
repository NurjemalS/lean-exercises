...
#eval eval someEnv (AExp.add (AExp.num 2) (AExp.var "x")) -- expected: 5
#eval eval someEnv (AExp.sub (AExp.var "y") (AExp.num 7)) -- expected: 10
#eval eval someEnv (AExp.mul (AExp.var "x") (AExp.var "y")) -- expected: 51

#eval eval someEnv (AExp.div (AExp.var "y") (AExp.num 3)) -- expected: 5
#eval eval someEnv (AExp.div (AExp.var "y") (AExp.num 0)) -- division by zero
...
theorem map_id {α : Type} (xs : List α) :
  map (fun x ↦ x) xs = xs :=
  sorry
